...
AWS_SECRETS_MANAGER="$AWS_CLI --output json secretsmanager"
...
Optional arguments:
    ...
    --secrets-name               Secrets name from Secrets Manager
...
function assertRequiredArgumentsSet() {
    ...
    if [ -z ${AWS_DEFAULT_REGION+x} ];
      then unset AWS_DEFAULT_REGION
              ...
              AWS_SECRETS_MANAGER="$AWS_SECRETS_MANAGER --region $AWS_DEFAULT_REGION"
    fi
    if [ -z ${AWS_PROFILE+x} ];
      then unset AWS_PROFILE
      else
              AWS_ECS="$AWS_ECS --profile $AWS_PROFILE"
              AWS_SECRETS_MANAGER="$AWS_SECRETS_MANAGER --profile $AWS_PROFILE"
    fi
...
function getSecrets() {
    if [ $SECRETS_NAME != false ]; then
      # Get secrets
      SECRETS=$(jq -n --argfile secrets_manager_secrets <($AWS_SECRETS_MANAGER get-secret-value --secret-id "$SECRETS_NAME") '$secrets_manager_secrets.SecretString | fromjson | to_entries | map( {"name": .key , "valueFrom": ($secrets_manager_secrets.ARN + ":" + .key + "::") })')
    fi
}
...
function createNewTaskDefJson() {
    ...
    if [ "$SECRETS" != false ]; then
      # update secrets of every task definition
      NEW_DEF=$(jq -n --argjson secrets "$SECRETS" --argjson task_definition "$NEW_DEF" '$task_definition | .containerDefinitions[].secrets |= $secrets')
    fi
